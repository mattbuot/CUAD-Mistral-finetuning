...
def create_fine_tuning_job(client: Mistral,
...
def single_inference(client: Mistral, model: str, examples: list[dict[str, str]]) -> list[str]:
    ...
    for i, example in enumerate(examples):
        ...
        final_predictions.append(json.loads(response.choices[0].message.content)["highlighted"])
...
def batch_inference(client: Mistral, model: str, examples: list[list[dict[str, str]]]) -> list[str]:
    ...
    batch_data = client.files.upload(
        file={"file_name": "data/batch_input.jsonl", "content": open("data/batch_input.jsonl", "rb")},
        purpose="batch"
    )
    ...
    final_predictions = utils.parse_batch_results('data/batch_results.jsonl', n_queries=len(examples))
    return final_predictions
